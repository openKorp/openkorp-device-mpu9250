...
openkorp::logic::Quaternion msg;
opendlv::proxy::AltitudeReading test;
...
static void cb_func(void)
{
	if(show_quat && enable_mag){
		...
		// openkorp::logic::Quaternion msg;
		msg.w(static_cast<float>(data.fused_quat[QUAT_W])).x(static_cast<float>(data.fused_quat[QUAT_X])).y(static_cast<float>(data.fused_quat[QUAT_Y])).z(static_cast<float>(data.fused_quat[QUAT_Z]));
		od4.send(msg);
		test.altitude(0);
		od4.send(test);
	}
...
int main(int argc, char *argv[])
{
	...
	// __print_header();
 	// od4 = cluon::OD4Session(112);

	if(!silent_mode) rc_mpu_set_dmp_callback(&cb_func);
