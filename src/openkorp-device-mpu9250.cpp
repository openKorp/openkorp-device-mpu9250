...
int32_t main(int32_t argc, char **argv) {
  ...
  if (0 == commandlineArguments.count("cid") 
    ...
    std::cerr << argv[0] << " interfaces to the mpu9250 imu sensor." << std::endl;
    std::cerr << "Usage:   " << argv[0] << " --cid=<conference id> [--verbose=<[0-9]>]" << std::endl;
    std::cerr << "Example: " << argv[0] << " --cid=111 --verbose=2" << std::endl;
    return 1;
  } 
  ...
  uint16_t const CID = std::stoi(commandlineArguments["cid"]);
	...
	{
		quaternionMsg.w(static_cast<float>(data.fused_quat[QUAT_W])).x(static_cast<float>(data.fused_quat[QUAT_X])).y(static_cast<float>(data.fused_quat[QUAT_Y])).z(static_cast<float>(data.fused_quat[QUAT_Z]));
